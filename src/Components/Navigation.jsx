  ...
  return (
    <header className='absolute inset-x-0 top-0 z-50'> 
      ...
      <Dialog open={mobileMenuOpen} onClose={setMobileMenuOpen} className='lg:hidden'>
        <div className='fixed inset-0 z-50 bg-slate-300/50 transition-opacity' />
        
        <DialogPanel className='fixed inset-0 mx-auto z-50 w-11/12 h-1/2 rounded-2xl overflow-y-auto bg-white px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10'>
              ...
              <div className='space-y-2 py-6'>
                {navigation.map((item) => (
                  <a
                    key={item.name}
                    href={item.href}
                    className='-mx-3 block rounded-lg px-3 py-1 font-semibold text-gray-900 hover:bg-gray-50'
                  >
                    {item.name}
                  </a>
                ))}
